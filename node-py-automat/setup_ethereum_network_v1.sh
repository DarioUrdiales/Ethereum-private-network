...
RED_COUNT=$(jq '.reds | length' "$JSON_FILE")
echo "Number of Reds: $RED_COUNT"

# Example of extracting other values (for the first red)
if [ "$RED_COUNT" -gt 0 ]; then
    # Extract miner IP for the first red
    MINER_IP=$(jq -r '.reds[0].nodes.miner.ip' "$JSON_FILE")
    echo "Miner IP for first red: $MINER_IP"

    # Extract RPC nodes information
    RPC_NODES=$(jq -c '.reds[0].nodes.rpc[]' "$JSON_FILE")
    for rpc in $RPC_NODES; do
        RPC_NAME=$(echo $rpc | jq -r '.name')
        RPC_PORT=$(echo $rpc | jq -r '.port')
        RPC_IP=$(echo $rpc | jq -r '.ip')
        echo "RPC Node: $RPC_NAME, Port: $RPC_PORT, IP: $RPC_IP"
    done
fi
...
services:
  geth-bootnode:
    ...
    image: geth-client
    # data: 
    #   --ipcpath /Users/jackyb/shortgeth/geth.ipc
    build:
      args:
  ...
  geth-miner:
    ...
    command:
      ...
      --unlock \${SIGNER_ADDRESS} --password password.txt:/root/.ethereum/password.sec
      --networkid=\${CHAIN_ID}
      --netrestrict="\${SUBNET}"
    networks:
      priv-eth-net:
