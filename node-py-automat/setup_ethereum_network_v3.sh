...
# Process each red
for ((red_index = 0; red_index < RED_COUNT; red_index++)); do
    # Extract Red ID
    RED_ID=$(jq -r ".reds[$red_index].id" "$JSON_FILE")

    # Create directory for the red
    DATA_DIR="${PWD}/data/${RED_ID}"
    mkdir -p "${DATA_DIR}"

    # Step 1: Generate Ethereum accounts
    echo "Generating Ethereum accounts for ${RED_ID}..."
    SIGNER_ADDRESS_FILE=$(docker run --rm -v "${DATA_DIR}:/root" ethereum/client-go:latest account new --password /root/password.txt --datadir /root/miner-accounts)
    ...
    DOCKER_COMPOSE_FILE="${DATA_DIR}/docker-compose.yaml"
    ...
    # Display information
    echo "Ethereum ${RED_ID} started. You can access the RPC interface at http://localhost:${IPRPC}"
