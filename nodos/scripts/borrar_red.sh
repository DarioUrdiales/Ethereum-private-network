...
carpeta="../blockchain-$CHAIN_ID"

if [ -d "$carpeta" ]; then
  cd ../blockchain-$CHAIN_ID && docker-compose down
  echo "La red de la blockhain con id 7000 ha sido eliminada."
else
  echo "No se ha eliminado la red porque no existe."
fi
